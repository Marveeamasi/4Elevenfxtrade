...
export default function Sidebar({title}) {
  ...
  return (
    <div className='flex sm:flex-col sm:sticky sm:top-0 sm:left-0 shadow-dark sm:shadow-dark-sm max-sm:justify-around max-sm:items-center sm:pl-5 sm:py-5 sm:w-[200px] w-full sm:h-screen h-auto bg-[#0a0c0c] max-sm:fixed bottom-0 z-[11]'>
        <Link href={'/dashboard'} className='flex items-center max-sm:hidden font-semibold mb-10 text-col'>4Elevenfxtrade</Link>
        {updatedNav.length>0 && updatedNav.map((n) => (
         ...
         {n.name==='dashboard' && <MdDashboard className='text-[24px] max-xsm:text-[20px]'/>}
         {n.name==='invest' && <FaCoins className='text-[24px] max-xsm:text-[20px]'/>}
         {n.name==='currents' && <GrTransaction className='text-[24px] max-xsm:text-[20px]'/>}
         {n.name==='requests' && <MdRequestQuote className='text-[24px] max-xsm:text-[20px]'/>}
         {n.name==='withdraw' && <PiHandWithdrawFill className='text-[24px] max-xsm:text-[20px]'/>}
          </Link>
         ))}
